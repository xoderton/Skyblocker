...
public class ColorTooltip extends SimpleTooltipAdder {
    ...
    private static boolean isException(String id, String hex) {
        return switch (id) {
            case String it when it.startsWith("LEATHER") || it.equals("GHOST_BOOTS") || Constants.SEYMOUR_IDS.contains(it) ->
                    true;
    ...
    private static DyeType checkDyeType(ItemStack stack, String hex) {
        return switch (hex) {
            ...
            case String it when Constants.SPOOK.contains(it) && stack.getSkyblockId().startsWith("FAIRY_") ->
                    DyeType.SPOOK;
    ...
    private static boolean isWitherGlitched(String id, String hex, long obtained) {
        ...
        return switch (id) {
            case String it when it.contains("CHESTPLATE") ->
                    Constants.WITHER_CHESTPLATE_HEXES.containsKey(hex) && Constants.WITHER_CHESTPLATE_HEXES.containsValue(it) && !Constants.WITHER_CHESTPLATE_HEXES.get(hex).equals(it);
            case String it when it.contains("LEGGINGS") ->
                    Constants.WITHER_LEGGINGS_HEXES.containsKey(hex) && Constants.WITHER_LEGGINGS_HEXES.containsValue(it) && !Constants.WITHER_LEGGINGS_HEXES.get(hex).equals(it);
            case String it when it.contains("BOOTS") ->
                    Constants.WITHER_BOOTS_HEXES.containsKey(hex) && Constants.WITHER_BOOTS_HEXES.containsValue(it) && !Constants.WITHER_BOOTS_HEXES.get(hex).equals(it);
