...
public class SupercraftReminder extends SimpleTooltipAdder {
    ...
    public void addToTooltip(@Nullable Slot focusedSlot, ItemStack stack, List<Text> lines) {
        ...
        if (!uuid.isEmpty())
            return; //Items with UUID can't be stacked, and therefore the shift-click feature doesn't matter
        int index = lines.size() - 1;
        if (lines.get(lines.size() - 2).getString().equals("Recipe not unlocked!"))
            index--; //Place it right below the "Right-Click to set amount" line
