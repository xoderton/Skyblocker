...
public class DungeonScore {
    ...
    public static void init() {
        ...
        ClientReceiveMessageEvents.GAME.register((message, overlay) -> {
            ...
            if (!dungeonStarted) {
                ...
                if (floorHasMimics)
                    checkMessageForMimic(str); //Only called when the message is not cancelled & isn't on the action bar, complementing MimicFilter
            }
        });
    ...
    private static int calculateScore() {
        if (isCurrentFloorEntrance)
            return Math.round(calculateTimeScore() * 0.7f) + Math.round(calculateExploreScore() * 0.7f) + Math.round(calculateSkillScore() * 0.7f) + Math.round(calculateBonusScore() * 0.7f);
    ...
    private static int calculateBonusScore() {
        ...
        if (getSecretsPercentage() >= 100 && floorHasMimics)
            mimicScore = 2; //If mimic kill is not announced but all secrets are found, mimic must've been killed
        return paulScore + cryptsScore + mimicScore;
    }

    public static boolean isEntityMimic(Entity entity) {
        if (!Utils.isInDungeons() || !floorHasMimics || !(entity instanceof ZombieEntity zombie) || !zombie.isBaby())
            return false;
        try {
    ...
    private static int getCrypts() {
        ...
        if (matcher == null)
            matcher = PlayerListMgr.regexAt(32, CRYPTS_PATTERN); //If class milestone 9 is reached, crypts goes up by 1
    ...
    private static void checkMessageForDeaths(String message) {
        ...
        final String whoDied = matcher.group("whodied").transform(s -> {
            if (s.equals("You"))
                return MinecraftClient.getInstance().getSession().getUsername(); //This will be wrong if the dead player is called 'You' but that's unlikely
            else return s;
        });
